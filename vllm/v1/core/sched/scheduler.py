...
class Scheduler(SchedulerInterface):
    ...
    def _should_pad_spec_decode(
        self, request: Request, num_computed_tokens: int
    ) -> bool:
        """Whether to pad a single (first) decode step with placeholder spec
        tokens (-1), to ensure uniform decode batches that use full cudagraphs.
        """
        return (
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            batch_has_prefill |= request.num_computed_tokens < request.num_prompt_tokens
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if load_kv_async:
                    ...
                    pad_spec_decode = (
                        not batch_has_prefill
                        and self._should_pad_spec_decode(request, num_computed_tokens)
                    )
                    if pad_spec_decode:
                    ...
                    if num_new_tokens > token_budget:
                        if pad_spec_decode:
                            break  # Don't break uniform decode size.
