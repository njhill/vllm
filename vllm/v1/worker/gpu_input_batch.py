...
class InputBatch:
    def __init__(
        ...
        # per in-flight pipeline stage. Indexed by `pp_stream_index`, which is
        # advanced once per `execute_model` call so that step T writes the slot
        # that step T + pp_size will read. Only populated for async scheduling
        # (where the previous step's sampled tokens are consumed via GPU rather
        # than re-sent through the scheduler).
