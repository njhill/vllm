...
class MooncakeStoreWorker:
    ...
    def start_load_kv(
        ...
        Invoked after the forward launch (no sync loads) for
        compute-I/O overlap, with KVConnectorLoadGate ordering the loads
        after any in-flight KV block zeroing.
        """
        if self._capacity_only:
            return
    ...
    def wait_for_save(
        ...
        if self.kv_role not in ["kv_producer", "kv_both"]:
            return
